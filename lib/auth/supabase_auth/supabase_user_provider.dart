...
class EcogasSupabaseUser extends BaseAuthUser {
  EcogasSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> ecogasSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = EcogasSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
